...
internal struct ECDSASigner: JWTAlgorithm, OpenSSLSigner {
    ...
    var curveResultSize: Int {
        let curveName = CCryptoBoringSSL_EC_GROUP_get_curve_name(CCryptoBoringSSL_EC_KEY_get0_group(key.c))
        switch curveName {
        ...
        where Plaintext: DataProtocol
    {
        ...
        guard let signature = CCryptoBoringSSL_ECDSA_do_sign(
            digest,
        ...
        defer { CCryptoBoringSSL_ECDSA_SIG_free(signature) }

        // serialize r+s values
        // see: https://tools.ietf.org/html/rfc7515#appendix-A.3
        let r = CCryptoBoringSSL_ECDSA_SIG_get0_r(signature)
        let s = CCryptoBoringSSL_ECDSA_SIG_get0_s(signature)
        let rsSize = self.curveResultSize
        var rBytes = [UInt8](repeating: 0, count: rsSize)
        var sBytes = [UInt8](repeating: 0, count: rsSize)
        let rCount = Int(CCryptoBoringSSL_BN_bn2bin(r, &rBytes))
        let sCount = Int(CCryptoBoringSSL_BN_bn2bin(s, &sBytes))
        ...
        where Signature: DataProtocol, Plaintext: DataProtocol
    {
        ...
        let signature = CCryptoBoringSSL_ECDSA_SIG_new()
        defer { CCryptoBoringSSL_ECDSA_SIG_free(signature) }
        ...
        try signatureBytes.prefix(rsSize).withUnsafeBufferPointer { r in
            try signatureBytes.suffix(rsSize).withUnsafeBufferPointer { s in
                ...
                guard CCryptoBoringSSL_ECDSA_SIG_set0(
                    signature,
                    CCryptoBoringSSL_BN_bin2bn(r.baseAddress, rsSize, nil),
                    CCryptoBoringSSL_BN_bin2bn(s.baseAddress, rsSize, nil)
                ) == 1 else {
                    throw JWTError.signingAlgorithmFailure(ECDSAError.signFailure)
                }
            }
        ...
        return CCryptoBoringSSL_ECDSA_do_verify(
            digest,
            numericCast(digest.count),
            signature,
            self.key.c
